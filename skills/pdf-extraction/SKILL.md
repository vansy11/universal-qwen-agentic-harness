...
- APPLY: Use the absolute path to `~/.qwen/core/pdf-reader.py` (resolve `~` to the user home) via `run_shell_command` to read local PDFs.
...
2. You MUST use `run_shell_command` to execute Python with the ABSOLUTE path:
   `python ~/.qwen/core/pdf-reader.py` (resolve `~` to the user home)
3. Pass the JSON input via stdin:
   `echo {"file_path": "/path/to/document.pdf"} | python ~/.qwen/core/pdf-reader.py`
